...
// Wrapped in React `cache()` so the layout, generateMetadata, and the page
// component all share a SINGLE DB query per request instead of issuing three
// independent `findById/findOne` + populate round-trips. Cache is request-scoped.
export const loadBoardgame = cache(async (idOrSlug) => {
...
export const loadExpansion = cache(async (idOrSlug) => {
  if (!idOrSlug) return null;
  await connectToDB();
