...
const V2_TOPK = 10;
// Permissive floor only — the LLM reranker (Gemini Flash) is the real
// relevance gate. A higher threshold (e.g. 0.2) silently dropped borderline
// matches for short generic queries like "who wins in a tie" even when the
// chunk genuinely answered the question, which then triggered the misleading
// "rulebook not uploaded" branch in app/api/chat/route.js.
const V2_SCORE_THRESHOLD = 0.05;
const V2_VECTOR_DIM = 1536;
const RERANK_DEFAULT_N = 3;
...
// @returns {Promise<{ chunks: Chunk[], legendChunks: Chunk[] }>}
//
...
// --------------------------------------------------------------------------
export async function retrieveForChat({ client, query, boardgames }) {
  if (!boardgames?.length) return { chunks: [], legendChunks: [] };
  ...
  const [v1Chunks, v2Result] = await Promise.all([
    ...
    v2.length > 0 ? retrieveV2Multi(client, query, v2) : Promise.resolve({ chunks: [], legendChunks: [] }),
  ]);
...
async function retrieveV2Multi(client, query, games) {
  ...
  const chunks = mainResp.matches
    .filter((m) => m.score >= V2_SCORE_THRESHOLD)
    .map(mapV2Match);
...
// --------------------------------------------------------------------------
export async function rerankChunks(query, chunks, n = RERANK_DEFAULT_N) {
...
${numbered}`;
  ...
  try {
    ...
    if (!match) return chunks.slice(0, n);
