...
4. **Page boundaries — EVERY page must appear in your output.**
   - Insert a line \`<!-- page N -->\` at the START of EVERY page in this excerpt. Use the ORIGINAL page numbers given in the "Page numbering for THIS excerpt" section below. Never use page numbers outside the range provided.
   - For EACH page, scan for transcribable prose: paragraphs, headers, lists, tables, callout/sidebar boxes, captions that read as prose (not labels inside an illustration). If ANY exists, transcribe ALL of it under that page's marker.
   - If a page has zero transcribable prose (e.g. a pure cover-art page or a full-bleed illustration with no readable rule text), still emit the marker followed by exactly this italic placeholder on its own line: \`_(no rule text on this page)_\`. This proves you saw the page and chose to skip it — never silently omit a page marker.
   - Self-check before finishing: count the \`<!-- page N -->\` markers in your output. The count MUST equal the number of pages in the excerpt. If any are missing, go back and add them with their content.
...
(e) **Ignore the IMAGE itself — never the page it's on.** If a graphic isn't a repeating icon defined in the Iconography legend (illustration, example board, card mockup, decorative artwork, photograph), don't describe it and don't transcribe text that is rendered INSIDE the illustration (e.g. labels painted onto a card mockup, captions on a depicted board piece). BUT always transcribe ordinary prose on the same page — paragraphs, headers, lists, tables, sidebars, and rule callouts are prose, not "images," and must be transcribed verbatim even when surrounded by illustrations. A "page with images" is not the same as "a page to skip."
...
- Don't skip pages. The only valid reason to omit prose from a page is if that page genuinely contains no prose to transcribe (all illustration, no readable rule text); in that case, still emit the page marker with the \`_(no rule text on this page)_\` placeholder. If a page has paragraphs, headers, lists, tables, callouts, sidebars, or "Example:" prose, that's transcribable text and skipping it is wrong.
...
// Lines that introduce / belong to a markdown block element and must NEVER
// be merged into adjacent prose — neither swallowed by the line above nor
// swallowing the line below. Covers headings (including no-space `###Foo`),
// page-marker comments, list items, ordered list items, table rows,
// blockquotes, and code fences.
const STRUCTURAL_LINE = /^(#{1,6}|<!--|[-*+]\s|\d+\.\s|\|\s?|>\s|```)/;
 ...
 * and code fences.
 *
 * Line-by-line pass so we can check BOTH the previous line and the current
 * line before deciding to merge. The earlier single-pass regex only checked
 * the line being merged INTO (the lookahead), which meant a structural line
 * like `<!-- page 2 -->` or `### Heading` could swallow the prose on the
 * following line. That mangled chunks for whole pages — the chunker saw the
 * merged page-marker line and `continue`d past the prose, or treated the
 * merged heading+body as a single heading with no body content.
 */
...
/**
 * Strip the `_(no rule text on this page)_` placeholder lines that the prompt
 * asks Gemini to emit for image-only pages. The page marker above the line
 * is preserved — it proves Gemini saw the page — but the placeholder text
 * itself would otherwise become a useless one-line chunk.
 */
export function stripPagePlaceholders(markdown) {
  if (!markdown) return markdown;
  return markdown
    .split("\n")
    .filter((line) => line.trim() !== "_(no rule text on this page)_")
    .join("\n");
}

/**
 * Drop any `<!-- page N -->` comments where N is outside `[startPage, endPage]`.
 *
 * Gemini occasionally confuses the slice's internal page numbers with the
 * printed page number visible on the page itself, producing markers that
 * point well outside the actual excerpt. Stripping the bad markers lets the
 * downstream chunker fall back to the last good marker (the content "belongs
 * to the previous valid page"), which is far less wrong than silently
 * landing on a page hundreds of pages off.
 */
export function clampPageMarkers(markdown, startPage, endPage) {
  if (startPage == null || endPage == null) return markdown;
  return markdown.replace(/<!--\s*page\s+(\d+)\s*-->/gi, (full, n) => {
    const num = Number(n);
    if (!Number.isFinite(num) || num < startPage || num > endPage) return "";
    return full;
  });
}

function buildPageRangeHint(startPage, endPage) {
  if (startPage == null || endPage == null) return "";
  if (startPage === endPage) {
    return `\n\n# Page numbering for THIS excerpt

This excerpt contains exactly ONE page from the original rulebook: page ${startPage}.
- Insert exactly one marker at the top: \`<!-- page ${startPage} -->\`.
- Never use any other page number.`;
  }
  return `\n\n# Page numbering for THIS excerpt

This excerpt contains pages ${startPage} through ${endPage} of the original rulebook (${endPage - startPage + 1} pages total).
- Insert a marker at the START of EVERY page in this excerpt, using the ORIGINAL page number.
- The first marker MUST be \`<!-- page ${startPage} -->\` (the very first thing in your output).
- The last marker MUST be \`<!-- page ${endPage} -->\`.
- Use only page numbers in the range ${startPage}–${endPage}. Never use any number outside this range.
- Emit exactly one marker per page. Do not skip any page, even if the page is mostly images or has little text.`;
}
...
/**
 * Pull the top-level (`##`) heading texts out of a batch's markdown so the
 * next batch can be told what sections are already established. Without this,
 * batches that start mid-section have no way to know whether a heading they
 * see is a continuation (`###` subsection) or a brand-new major section (`##`).
 *
 * Tolerant of `##Section` (no space) — Gemini sometimes omits the space.
 * Skips `#` (rulebook title) and `###` and deeper — only level-2 headings.
 */
export function extractSectionHeadings(markdown) {
  ...
  for (const line of markdown.split("\n")) {
    ...
    if (!m) continue;
    // Defensive: reject if line accidentally matched `###` (shouldn't, since
    // the negated class `[^#]` after `##` blocks that, but worth being clear).
    const text = m[1].trim();
...
function buildSectionContextHint(knownSections) {
  if (!knownSections?.length) return "";
  const bullets = knownSections.map((s) => `- ${s}`).join("\n");
  return `

# Section context from earlier pages

Earlier pages of this rulebook established these top-level (\`##\`) sections:
${bullets}

When parsing the pages in THIS excerpt:
- If a heading looks visually like a CONTINUATION or SUBSECTION of any section above (e.g. a smaller font/inset block under content that logically belongs to one of those sections), emit it as \`###\`, not \`##\`.
- If a heading is clearly a NEW top-level major section (visually prominent, peer to the ones above), emit it as \`##\` — don't demote it.
- Never invent a parent heading. If pages in this excerpt have only subsection content with no visible \`##\` heading of their own, just emit the \`###\` headings — the downstream chunker will attach them to the most recently established \`##\` from the stitched document.`;
}
 ...
 *   The ORIGINAL page number of the first page in this excerpt. When
 *   provided, the prompt tells Gemini to emit absolute (original) page
 *   markers, and any out-of-range markers Gemini returns are stripped as
 *   a safety net.
 * @param {number} [options.endPage]
 *   The ORIGINAL page number of the last page in this excerpt.
 ...
 * @param {string[]} [options.knownSections=[]]
 *   `##`-level section headings already established by earlier batches.
 *   Appended to the prompt so this batch knows to emit `###` for
 *   subsections instead of promoting them to `##` when their parent
 *   heading isn't visible on the current pages.
...
export async function extractMarkdownFromBuffer(
  ...
  const prompt =
    EXTRACTION_PROMPT +
    buildPageRangeHint(startPage, endPage) +
    buildKnownTokensHint(knownIconTokens) +
    buildSectionContextHint(knownSections);
  ...
  markdown = clampPageMarkers(markdown, startPage, endPage);
  markdown = stripPagePlaceholders(markdown);
