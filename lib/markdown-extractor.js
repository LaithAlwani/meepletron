...
export function joinWrappedLines(markdown) {
  ...
  return markdown
    .replace(
      ...
      //
      // Note: the `#{1,6}` heading check does NOT require a trailing space —
      // Gemini sometimes emits "###Heading" with no space and that's still
      // intended as a heading. The chunker's heading regex below is
      // similarly tolerant. If we require `#{1,6}\s` we'd merge the heading
      // into the previous line as prose and lose the section break.
      /([^\n])\n(?!\n|#{1,6}|[-*+]\s|\d+\.\s|\|\s?|>\s|<!--|```)/g,
      (_, prev) => `${prev} `,
    )
    .replace(/[ \t]+/g, " ");
...
export async function extractMarkdownFromBuffer(
  pdfBuffer,
  { startPage = null, endPage = null, knownIconTokens = [] } = {},
) {
  const prompt =
    EXTRACTION_PROMPT +
