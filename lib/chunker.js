...
function parseSections(markdown) {
  ...
  for (const line of lines) {
    // Page-boundary comment. We also capture any trailing prose on the same
    // line as a safety net — `joinWrappedLines` should keep page markers on
    // their own line, but if anything (Gemini output, a future regression)
    // ever produces `<!-- page N --> some prose`, this prevents the chunker
    // from silently dropping that prose.
    const pageMatch = line.match(/^\s*<!--\s*page\s+(\d+)\s*-->(.*)$/);
    if (pageMatch) {
      currentPage = parseInt(pageMatch[1], 10);
      if (pendingPage === null) pendingPage = currentPage;
      const trailing = pageMatch[2].trim();
      if (trailing) pendingContent.push(trailing);
      continue;
    }
