            ...
            {urls.map((url, i) => (
              <li key={i}>
                <a
                  ...
                  {
                    url.path.split(
                      "https://meepletron-storage.s3.us-east-2.amazonaws.com/resources/",
                    )[1]
                  }
...
function InfoSection({ title, children }) {
  return (
    <section className="mb-8">
      <h2 className=" font-semibold uppercase tracking-widest text-subtle mb-3 pb-2 border-b border-border-muted">
        {title}
      </h2>
      {children}
    </section>
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {items.map((item) => (
        <span
          key={item}
          className="text-xs px-2.5 py-1 rounded-full bg-surface-muted text-muted border border-border">
          {item}
        </span>
      ))}
    </div>
