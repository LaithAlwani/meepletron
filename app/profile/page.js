    ...
    <div className="min-h-screen bg-[#f7f7f7] dark:bg-slate-900 pt-24 pb-16 px-4">
      <div className="max-w-xl mx-auto">
        ...
        <motion.div
          ...
          className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-8 mb-6 flex flex-col items-center text-center"
        >
          {/* Avatar */}
          {clerkUser?.imageUrl ? (
            <img
              ...
              className="w-20 h-20 rounded-full object-cover shadow-md mb-4 ring-2 ring-blue-100 dark:ring-yellow-500/30"
            />
          ) : (
            <div className="w-20 h-20 rounded-full bg-blue-100 dark:bg-yellow-500/20 flex items-center justify-center text-3xl font-bold text-blue-600 dark:text-yellow-400 mb-4">
          ...
          <button
            ...
            className="mt-6 flex items-center gap-2 px-5 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 text-sm font-medium text-gray-600 dark:text-slate-300 hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-600 dark:hover:text-red-400 hover:border-red-200 dark:hover:border-red-500/30 transition-all"
          >
            <MdLogout size={16} />
            Sign out
          </button>
        ...
        >
          <p className="text-xs uppercase tracking-widest text-gray-400 dark:text-slate-500 font-semibold mb-3 px-1">
            Activity
          </p>
            ...
            <StatCard
              ...
              sub={
                stats && ratingPct !== null
                  ? `${stats.correctRatings} correct · ${stats.wrongRatings} wrong`
                  : null
              }
              loading={loading}
              color="green"
...
function StatCard({ icon, label, value, sub, loading, color }) {
  ...
  return (
    <div className="flex flex-col gap-3 p-5 rounded-2xl bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 shadow-sm">
      <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${colors[color]}`}>
        {icon}
      </div>
      {loading ? (
        <div className="space-y-1.5">
          <div className="h-6 w-14 rounded-lg bg-gray-200 dark:bg-slate-700 animate-pulse" />
          <div className="h-3 w-20 rounded bg-gray-200 dark:bg-slate-700 animate-pulse" />
        </div>
      ) : (
        <div>
      ...
      <p className="text-xs text-gray-400 dark:text-slate-500 leading-tight">{label}</p>
    </div>
  );
}
