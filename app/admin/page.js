      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-2 gap-4 mb-10">
          ...
          <StatCard
            ...
            sub={
              stats?.aiMessages > 0
                ? `${Math.round((stats.unratedAI / stats.aiMessages) * 100)}% of AI responses`
                : null
            }
            loading={loading}
            color="amber"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Link
            ...
            className="group flex items-center gap-4 p-5 rounded-2xl bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 shadow-sm hover:shadow-md hover:border-blue-200 dark:hover:border-yellow-500/40 transition-all">
            <div className="w-11 h-11 rounded-xl bg-blue-100 dark:bg-yellow-500/15 flex items-center justify-center text-blue-600 dark:text-yellow-400 shrink-0 group-hover:scale-110 transition-transform">
              <MdMenuBook size={22} />
            </div>
            <div>
              <p className="font-semibold text-gray-900 dark:text-white text-sm">Board Games</p>
              <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">Add, edit and manage games</p>
            </div>
          </Link>
        </div>
    ...
    <div className="flex flex-col gap-3 p-5 rounded-2xl bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 shadow-sm">
      ...
      {loading ? (
        <div className="space-y-2">
          <div className="h-7 w-16 rounded-lg bg-gray-200 dark:bg-slate-700 animate-pulse" />
        ...
        <div>
          ...
          {sub && (
            <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5 tabular-nums">{sub}</p>
          )}
        </div>
      )}
